...
export const GoogleBooksSearch: React.FC<GoogleBooksSearchProps> = ({
  ...
  const handleImport = async (item: GoogleBookSearchResult) => {
    const primaryAuthor = item.authorName || (item.authors && item.authors.length > 0 ? item.authors[0] : 'Bilinmeyen Yazar');
    ...
    const dto: CreateBookDto = {
      ...
      authorDto: {
        name: primaryAuthor,
        bio: item.authors && item.authors.length > 1 ? `Ortak yazarlar: ${item.authors.join(', ')}` : undefined,
      },
    };
    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="border-b-2 border-on-background pb-4">
        <h1 className="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-background">
          Google Books Arşivi Arama
        </h1>
        <p className="font-body-lg text-body-lg text-on-surface-variant mt-1">
          Google Books çevrimiçi kütüphanesinde arama yapın ve eserleri doğrudan kütüphanenize aktarın.
        </p>
      </div>
      ...
      <form onSubmit={handleSearch}>
        <div className="bg-surface border-2 border-on-background rounded-xl p-4 sm:p-6 flex flex-col sm:flex-row gap-4 items-center shadow-brutal">
          <div className="relative flex-1 w-full">
            ...
            <input
              type="text"
              placeholder="Kitap başlığı, yazar, konu veya ISBN ile arama yapın..."
              value={query}
          ...
          >
            ...
            <span>{loading ? 'Aranıyor...' : 'Çevrimiçi Ara'}</span>
          </button>
        </div>
      </form>
      ...
      {loading ? (
        <div className="bg-surface border-2 border-on-background rounded-lg p-12 text-center shadow-brutal">
          ...
          <p className="font-label-md text-on-background font-bold">Google Books API sorgulanıyor...</p>
        </div>
      ) : results.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {results.map((book) => {
            const isImported = importedIds[book.id];
            const authorStr = book.authorName || (book.authors ? book.authors.join(', ') : 'Bilinmeyen Yazar');
            ...
            return (
              <div key={book.id} className="bg-surface border-2 border-on-background rounded-lg p-5 shadow-brutal-sm shadow-brutal-hover flex flex-col justify-between space-y-4">
                <div className="flex gap-4">
                  <div className="w-24 h-36 flex-shrink-0 bg-surface-dim border-2 border-on-background rounded overflow-hidden shadow-[2px_2px_0px_0px_#1e1c10]">
                    <img
                  ...
                  <div className="flex-1 space-y-1.5 overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 text-xs font-caption text-on-surface-variant pt-2">
                      <span className="flex items-center gap-1">
                        <FileText size={12} /> {book.pageCount || 250} Sayfa
                      </span>
                      <span className="flex items-center gap-1">
                ...
                <div className="pt-2 border-t-2 border-on-background/20">
                  {isImported ? (
                    <div className="flex items-center justify-center gap-2 py-2 text-xs font-label-md font-bold text-secondary">
                      <CheckCircle2 size={16} /> Kütüphaneye Eklendi
                    </div>
                  ) : (
                    <button
        ...
        <div className="bg-surface border-2 border-on-background rounded-lg p-10 text-center shadow-brutal">
          ...
          <p className="font-body-md text-on-background">"{query}" için Google Books üzerinde sonuç bulunamadı.</p>
        </div>
      ) : (
        <div className="bg-surface border-2 border-on-background rounded-lg p-12 text-center max-w-lg mx-auto shadow-brutal">
          ...
          <h3 className="font-headline-md text-headline-md text-on-background mb-2">Çevrimiçi Kütüphanede Ara</h3>
          <p className="font-body-md text-on-surface-variant text-sm">
            Google Books veritabanından kapak görselleri, yazar bilgileri ve sayfa sayılarını otomatik çekin.
          </p>
        </div>
      )}
    </div>
