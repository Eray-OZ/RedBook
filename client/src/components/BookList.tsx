      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant">
            Koleksiyonundaki tüm kitaplar, çizgi romanlar ve sesli kitaplar.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>Google Books İçe Aktar</span>
          </button>
          <button
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-primary-container border-2 border-on-background rounded-lg p-5 shadow-brutal flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <span className="bg-surface-bright text-on-background font-caption text-caption px-2 py-0.5 rounded-full border border-on-background font-bold">Koleksiyon</span>
          </div>
          <div className="mt-3">
            <p className="font-label-md text-label-md text-on-primary-container opacity-90">Toplam Kitap</p>
        ...
        <div className="bg-secondary-container border-2 border-on-background rounded-lg p-5 shadow-brutal flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <span className="bg-surface-bright text-on-background font-caption text-caption px-2 py-0.5 rounded-full border border-on-background font-bold">Hacim</span>
          </div>
          <div className="mt-3">
            <p className="font-label-md text-label-md text-on-secondary-container opacity-90">Toplam Sayfa</p>
            <p className="font-display-lg text-3xl font-bold text-on-secondary-container mt-0.5">{totalPages.toLocaleString('tr-TR')}</p>
          </div>
        </div>
        ...
        <div className="bg-tertiary-container border-2 border-on-background rounded-lg p-5 shadow-brutal flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <span className="bg-surface-bright text-on-background font-caption text-caption px-2 py-0.5 rounded-full border border-on-background font-bold">Sesli</span>
          </div>
          <div className="mt-3">
            <p className="font-label-md text-label-md text-on-tertiary-container opacity-90">Sesli Kitaplar</p>
        ...
        <div className="bg-surface border-2 border-on-background rounded-lg p-5 shadow-brutal flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <span className="bg-secondary-container text-on-secondary-container font-caption text-caption px-2 py-0.5 rounded-full border border-on-background font-bold">Çizgi</span>
          </div>
          <div className="mt-3">
            <p className="font-label-md text-label-md text-on-surface-variant">Çizgi Roman / Manga</p>
      ...
      <section className="space-y-4 pt-2">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-on-background pb-4">
          ...
          <div className="relative max-w-md w-full">
            ...
            <input
              type="text"
              placeholder="Kitap başlığı veya yazar adı ara..."
              value={searchQuery}
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
          {['ALL', 'Book', 'Comic', 'AudioBook', 'Other'].map((type) => (
            ...
            >
              {type === 'ALL' ? 'Tüm Formatlar' : type === 'Book' ? 'Kitap' : type === 'Comic' ? 'Çizgi Roman' : type === 'AudioBook' ? 'Sesli Kitap' : 'Diğer'}
            </button>
          ))}
        </div>
      </section>
      ...
      {filteredBooks.length > 0 ? (
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {filteredBooks.map((book) => {
            const authorDisplayName = book.authorName || book.author?.name || 'Bilinmeyen Yazar';
            const yearStr = book.publishYear ? new Date(book.publishYear).getFullYear() : 'N/A';

            return (
              <div
                key={book.id || book.title}
                className="bg-surface border-2 border-on-background rounded-lg p-3 shadow-brutal-sm shadow-brutal-hover flex flex-col group cursor-pointer transition-all"
              >
                <div className="w-full aspect-[2/3] border-2 border-on-background rounded bg-surface-dim relative overflow-hidden mb-3">
                  <img
                  ...
                  />
                  <div className="absolute top-2 right-2 bg-secondary text-on-secondary px-2 py-0.5 rounded-full border border-on-background font-caption text-[10px] transform rotate-3 shadow-[1px_1px_0px_0px_#1e1c10]">
                    {String(book.itemType)}
                  </div>
                </div>
                <h4 className="font-label-md text-label-md text-on-background line-clamp-1 font-bold">
                  {book.title}
                </h4>
                <p className="font-caption text-caption text-on-surface-variant line-clamp-1 mt-1 flex items-center gap-1">
                  <User size={12} className="text-primary" /> {authorDisplayName}
                </p>
                <div className="mt-3 pt-2 border-t border-on-background/20 flex items-center justify-between text-[11px] font-caption text-on-surface-variant">
                  <span className="flex items-center gap-1">
                    <Layers size={12} /> {book.defaultPageCount} syf
                  </span>
                  <span className="flex items-center gap-1">
                    <Calendar size={12} /> {yearStr}
                  </span>
                </div>
              </div>
            );
          ...
          <div
            onClick={onNavigateToAdd}
            className="bg-surface-variant/40 border-2 border-dashed border-on-background rounded-lg p-3 shadow-brutal-sm hover:bg-surface-variant flex flex-col items-center justify-center cursor-pointer transition-all group min-h-[220px]"
          >
            ...
            <h4 className="font-label-md text-label-md text-on-background text-center font-bold">Yeni Kitap Ekle</h4>
          </div>
        </section>
      ) : (
        <div className="bg-surface border-2 border-on-background rounded-lg p-12 text-center max-w-md mx-auto shadow-brutal">
          ...
          <h3 className="font-headline-md text-headline-md text-on-background mb-2">Eser Bulunamadı</h3>
          <p className="font-body-md text-on-surface-variant text-sm mb-6">
            Arama kriterlerinize eşleşen kitap bulunamadı. Kütüphanenize yeni bir eser ekleyebilirsiniz.
          </p>
          <button
            onClick={onNavigateToAdd}
